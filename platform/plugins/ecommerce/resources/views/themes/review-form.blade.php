...
{!! Form::open(['route' => 'public.reviews.create', 'method' => 'post']) !!}
    ...
    <div class="form-group {{ $errors->has('star') ? ' has-error' : '' }}">
        ...
        <input type="hidden" name="star" id="rating" @if (!auth('customer')->check() || ($is_reviewed = check_if_reviewed_product($productId))) disabled value="{{ get_user_reviewed_value($productId) }}" @endif/>
    </div>
    ...
    @if (!isset($is_reviewed) || !$is_reviewed)
        ...
        @else
            <div class="alert alert-success">
                <span>{!! trans('plugins/ecommerce::review.require_login', ['login' => Html::link(route('customer.login'), __('login'))]) !!}</span>
            </div>
        @endif
    @endif
{!! Form::close() !!}
            ...
            @foreach ($reviews as $review)
                @if (!empty($review->user) && !empty($review->product))
                    <li>
                        ...
                        @if (auth('customer')->check() && auth('customer')->user()->getAuthIdentifier() == $review->user_id)
