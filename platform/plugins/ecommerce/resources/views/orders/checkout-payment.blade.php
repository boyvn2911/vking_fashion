        ...
        <div class="row">
            <div class="col-lg-7 col-md-6 col-12 left">
                ...
                <ol class="breadcrumb d-none d-sm-flex">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('public.checkout.information', $token) }}">{{ __('Shipping information') }}</a></li>
                    <li class="breadcrumb-item">{{ __('Payment method') }}</li>
                </ol>

                <!-- for mobile device display -->
                <div class="d-sm-block d-md-none" style="padding: 0 15px;">
                    <div id="cart-item">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6 float-right">
                                <p class="total-text raw-total-text"
                                   data-price="{{ Cart::rawTotal() }}"> {{ format_price(Cart::rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount) }} </p>
                            </div>
                        </div>
                ...
                <div class="form-checkout">
                    @if (!empty($shipping))
                        <h5 class="checkout-payment-title">{{ __('Shipping') }}</h5>
                    ...
                    <form action="{{ route('payments.checkout') }}" method="post">
                        @csrf
                        <input type="hidden" name="amount" value="{{ Cart::rawTotal() - $promotionDiscountAmount - $couponDiscountAmount }}">
                        ...
                        <ul class="list-group list_payment_method">
                            ...
                            {!! apply_filters(PAYMENT_FILTER_ADDITIONAL_PAYMENT_METHODS, null, ['amount' => Cart::rawTotal() - $promotionDiscountAmount - $couponDiscountAmount, 'currency' => strtoupper(get_application_currency()->title), 'name' => $product->name]) !!}
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-12 col-12">
                                <button type="submit" class="btn payment-checkout-btn payment-checkout-btn-step float-right">
                                    {{ __('Checkout') }}
                                </button>
                            </div>
                        </div>
