@if ($productAttributeSets->count() > 0)
    <div class="add-new-product-attribute-wrap">
        ...
        <a href="#" class="btn-trigger-add-attribute" data-toggle-text="{{ __('Cancel') }}">{{ __('Add new attributes') }}</a>
        <p>{{ __('Adding new attributes helps the product to have many options, such as size or color.') }}</p>
                    ...
                    <div class="row">
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group">
                                <label class="text-title-field">{{ __('Attribute name') }}</label>
                        ...
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group">
                                <label class="text-title-field">{{ __('Value') }}</label>
        ...
        <div class="list-product-attribute-wrap hidden">
            ...
            <a href="#" class="btn btn-secondary btn-trigger-add-attribute-item @if ($productAttributeSets->count() < 2) hidden @endif">{{ __('Add more attribute') }}</a>
            @if ($product && $product->id)
                <a href="#" class="btn btn-info btn-trigger-add-attribute-to-simple-product" data-target="{{ route('products.add-attribute-to-product', $product->id) }}">{{ __('Continue') }}</a>
            @endif
        </div>
    </div>

@else
    <p>{!! __(':link to create product variations!', ['link' => Html::link(route('product-attribute-sets.create'), __('Add new attributes'))]) !!}</p>
@endif
