...
class CreateEcommerceTables extends Migration
{
    ...
    public function up()
    {
        Schema::create('ec_brands', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->integer('user_id')->nullable()->references('id')->on('users')->onDelete('set null');
        });
        ...
        Schema::create('ec_product_categories', function (Blueprint $table) {
            ...
            $table->integer('user_id')->nullable()->references('id')->on('users')->onDelete('set null');
            $table->integer('parent_id')->nullable()->references('id')->on('ec_product_categories')->onDelete('set null');
        });
        ...
        Schema::create('ec_products', function (Blueprint $table) {
            ...
            $table->integer('category_id')->unsigned()->nullable()->references('id')->on('ec_product_categories')->onDelete('set null');
            ...
            $table->integer('user_id')->nullable()->references('id')->on('users')->onDelete('set null');
            $table->integer('brand_id')->nullable()->references('id')->on('ec_brands')->onDelete('set null');
            $table->timestamps();
        });

        Schema::create('ec_product_category_product', function (Blueprint $table) {
            $table->id();
            $table->integer('category_id')->unsigned()->index()->references('id')->on('ec_product_categories')->onDelete('cascade');
            $table->integer('product_id')->unsigned()->index()->references('id')->on('ec_products')->onDelete('cascade');
        });
        ...
        Schema::create('ec_product_collection_products', function (Blueprint $table) {
            $table->id();
            $table->integer('product_collection_id')->unsigned()->index()->references('id')->on('ec_product_collections')->onDelete('cascade');
            $table->integer('product_id')->unsigned()->index()->references('id')->on('ec_products')->onDelete('cascade');
        });
        ...
        Schema::create('ec_product_attribute_sets', function (Blueprint $table) {
            ...
            $table->string('status', 60)->default('published');
            $table->tinyInteger('order')->unsigned()->default(0);
            $table->integer('created_by')->unsigned()->nullable()->references('id')->on('users')->onDelete('set null');
            $table->integer('updated_by')->unsigned()->nullable()->references('id')->on('users')->onDelete('set null');
            $table->timestamps();
        });

        Schema::create('ec_product_attributes', function (Blueprint $table) {
            $table->id();
            $table->integer('attribute_set_id')->unsigned()->references('id')->on('ec_product_attribute_sets')->onDelete('cascade');
            $table->string('title', 120);
            $table->string('slug', 120)->nullable();
        ...
        Schema::create('ec_product_with_attribute_set', function (Blueprint $table) {
            $table->id();
            $table->integer('attribute_set_id')->unsigned()->references('id')->on('ec_product_attribute_sets')->onDelete('cascade');
            $table->integer('product_id')->unsigned()->references('id')->on('ec_products')->onDelete('cascade');
            $table->tinyInteger('order')->unsigned()->default(0);
        });

        Schema::create('ec_product_with_attribute', function (Blueprint $table) {
            $table->id();
            $table->integer('attribute_id')->unsigned()->references('id')->on('ec_product_attributes')->onDelete('cascade');
            $table->integer('product_id')->unsigned()->references('id')->on('ec_products')->onDelete('cascade');
        });

        Schema::create('ec_product_variations', function (Blueprint $table) {
            $table->id();
            $table->integer('product_id')->unsigned()->nullable()->references('id')->on('ec_products')->onDelete('set null');
            $table->integer('configurable_product_id')->unsigned()->references('id')->on('ec_products')->onDelete('cascade');
            $table->tinyInteger('is_default')->default(0);
        });
        ...
        Schema::create('ec_product_variation_items', function (Blueprint $table) {
            $table->id();
            $table->integer('attribute_id')->unsigned()->references('id')->on('ec_product_attributes')->onDelete('cascade');
            $table->integer('variation_id')->unsigned()->references('id')->on('ec_product_variations')->onDelete('cascade');
        });
        ...
        Schema::create('ec_order_product', function (Blueprint $table) {
            $table->id();
            $table->integer('order_id')->unsigned()->references('id')->on('ec_orders')->onDelete('cascade');
            $table->integer('qty');
            $table->decimal('price', 15, 2);
            $table->decimal('tax_amount', 15, 2);
            $table->text('options')->nullable(); // Where to store product versions information
            $table->integer('product_id')->unsigned()->nullable()->references('id')->on('ec_products')->onDelete('set null');
        ...
        Schema::create('ec_discounts', function (Blueprint $table) {
            ...
            $table->boolean('can_use_with_promotion')->default(false)->nullable();
            $table->string('discount_on', 20)->nullable();
            $table->integer('product_quantity', false, true)->nullable();
            $table->string('type_option', 100)->default('amount')->nullable();
            $table->string('target', 100)->default('all-orders')->nullable();
        ...
        Schema::create('ec_wish_lists', function (Blueprint $table) {
            $table->id();
            $table->integer('customer_id')->unsigned()->references('id')->on('users')->onDelete('cascade');
            $table->integer('product_id')->unsigned()->references('id')->on('ec_products')->onDelete('cascade');
            $table->timestamps();
        });
        ...
        Schema::create('ec_grouped_products', function (Blueprint $table) {
            $table->id();
            $table->integer('parent_product_id')->unsigned()->references('id')->on('ec_products')->onDelete('cascade');
            $table->integer('product_id')->unsigned()->references('id')->on('ec_products')->onDelete('cascade');
        ...
        Schema::create('ec_product_related_relations', function (Blueprint $table) {
            $table->engine = 'InnoDB';
            $table->id();
            $table->integer('from_product_id')->unsigned()->index()->references('id')->on('ec_products')->onDelete('cascade');
            $table->integer('to_product_id')->unsigned()->index()->references('id')->on('ec_products')->onDelete('cascade');
        });
