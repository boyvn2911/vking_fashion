...
if (!function_exists('check_if_reviewed_product')) {
     ...
     * @return bool
     */
    function check_if_reviewed_product($productId, $userId = null)
    {
        if ($userId == null && auth('customer')->check()) {
            $userId = auth('customer')->user()->getAuthIdentifier();
        }
...
if (!function_exists('get_user_reviewed_value')) {
    /**
     * @param int $productId
     * @param int $userId
     * @return int
     */
    function get_user_reviewed_value($productId, $userId = null)
    {
        if ($userId == null && auth('customer')->check()) {
            $userId = auth('customer')->user()->getAuthIdentifier();
        }
...
if (!function_exists('get_average_star_of_product')) {
    ...
    {
        return (float)app(ReviewInterface::class)->getModel()
