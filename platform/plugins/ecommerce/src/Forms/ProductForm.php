...
class ProductForm extends FormAbstract
{
    ...
    public function buildForm()
    {
        ...
        $productAttributeSets = app(ProductAttributeSetInterface::class)->getAllWithSelected($this->getModel() ? $this->getModel()->id : null);
        $productAttributes = app(ProductAttributeInterface::class)->getAllWithSelected($this->getModel() ? $this->getModel()->id : null);
        ...
        if ($this->getModel()) {
            ...
            $productsRelatedToVariation = app(ProductInterface::class)
                ->getProductVariations($this->getModel()->id);
        }
        ...
        $this
            ...
            ->addMetaBoxes([
                'image'          => [
                    'title'    => trans('plugins/ecommerce::products.form.image'),
                    'content'  => view('plugins/ecommerce::products.partials.images',
                        ['product' => $this->getModel()->id ? $this->getModel() : null])->render(),
                    'priority' => 0,
                ],
                'image-template' => [
                    'title'    => null,
                    'content'  => view('plugins/ecommerce::products.partials.select-image-template')->render(),
                    'wrap'     => false,
                    'priority' => 1,
                ],
                'with_related'   => [
                    ...
                    'content'  => '<div class="wrap-relation-product" data-target="' . route('products.get-relations-boxes',
                            $this->getModel()->id ? $this->getModel()->id : 0) . '"></div>',
                    'wrap'     => false,
                    'priority' => 9999,
                ],
            ])
                ...
                ->addMetaBoxes([
                    ...
                    'attributes' => [
                        ...
                        'content'       => view('plugins/ecommerce::products.partials.add-product-attributes', [
                            ...
                            'product'              => $this->getModel()->id ? $this->getModel() : null,
                        ])->render(),
                        'after_wrapper' => '</div>',
                        'priority'      => 3,
                    ],
                ]);
        } elseif ($this->getModel()->id) {
            $this
