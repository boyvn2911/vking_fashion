...
class PublicController extends Controller
{
    ...
    public function getEditAccount()
    {
        ...
        Theme::asset()
            ->add('datepicker-style', 'vendor/core/libraries/bootstrap-datepicker/css/bootstrap-datepicker3.min.css',
                ['bootstrap']);
        Theme::asset()
            ->container('footer')
            ->add('datepicker-js', 'vendor/core/libraries/bootstrap-datepicker/js/bootstrap-datepicker.min.js',
                ['jquery']);
    ...
    {
        $this->customerRepository->createOrUpdate($request->input(),
    ...
    public function postChangePassword(UpdatePasswordRequest $request, BaseHttpResponse $response)
    {
        ...
        $this->customerRepository->update(['id' => $currentUser->getKey()], [
    ...
    {
        SeoHelper::setTitle(__('Wish Lists'));
        $wish_lists = $this->wishListRepository->allBy(['customer_id' => auth('customer')->user()->getAuthIdentifier()],
            ['product']);

        return Theme::scope('ecommerce.customers.wish-list', compact('wish_lists'),
            'plugins/ecommerce::themes.customers.wish-list')->render();
    }
     ...
     * @return BaseHttpResponse
     */
    public function addToWishList($productId, BaseHttpResponse $response)
    {
        $product = $this->productRepository->findOrFail($productId);
        if (is_added_to_wish_lists($productId)) {
            return $response->setError()
                ->setMessage(trans('plugins/ecommerce::messages.wishlist.add_to_wishlist_fail',
        ...
        return $response->setMessage(trans('plugins/ecommerce::messages.wishlist.add_to_wishlist_success',
            compact('product')));
    }
     ...
     * @return BaseHttpResponse
     */
    public function removeWishListItem($productId, BaseHttpResponse $response)
    {
        ...
        return $response->setMessage(trans('plugins/ecommerce::messages.wishlist.remove_success'));
    }
    ...
    public function postCreateAddress(AddressRequest $request, BaseHttpResponse $response)
    {
        ...
        return $response
            ->setData([
                'id'   => $address->id,
                'html' => view('plugins/ecommerce::themes.customers.address.partials.address-item',
    ...
    public function postEditAddress($id, AddressRequest $request, BaseHttpResponse $response)
    {
        ...
        return $response
            ->setData([
                'id'   => $address->id,
                'html' => view('plugins/ecommerce::themes.customers.address.partials.address-item', compact('address'))
                    ->render(),
            ])
