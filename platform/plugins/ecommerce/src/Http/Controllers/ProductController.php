...
class ProductController extends BaseController
{
    ...
    public function postSaveAllVersions(
        ...
        foreach ($versionInRequest as $variationId => $version) {
            ...
            if (!$variation->product_id || $isUpdateProduct) {
                ...
                $images = json_encode(Arr::get($version, 'images', []));
                $productRelatedToVariation->images = $images == '[null]' ? '[]' : $images;
    ...
    public function update(
        ...
        if ($request->input('is_added_attributes') == 1 && $addedAttributes) {
            ...
            $variation['sku'] = $product->sku ?? time();
            foreach (array_keys($addedAttributes) as $attribute_id) {
                $attribute = $this->productAttributeRepository->findById($attribute_id);
                if ($attribute) {
                    $variation['sku'] .= '-' . $attribute->slug;
                }
            }
    ...
    public function postAddVersion(
        ...
        if ($addedAttributes && !empty($addedAttributes) && is_array($addedAttributes)) {
            ...
            if (!$result['created']) {
                return $response
                    ->setError()
                    ->setMessage(__('This variation is not existed.'));
            }
        }
    ...
    public function postUpdateVersion(
        ...
        if ($variation->is_default) {
            ...
            $this->productRepository->createOrUpdate($request->input(), ['id' => $variation->configurable_product_id]);
        }
    ...
    public function getRelationBoxes($id, BaseHttpResponse $response)
    {
        ...
        return $response->setData(view('plugins/ecommerce::products.partials.extras', compact('product', 'onlyRelatedProducts'))->render());
    }
    ...
    public function postCreateProductWhenCreatingOrder(
        ...
        $product->image_url = RvMedia::getImageUrl(Arr::first($product->images) ?? null, 'thumb', false, RvMedia::getDefaultImage());
