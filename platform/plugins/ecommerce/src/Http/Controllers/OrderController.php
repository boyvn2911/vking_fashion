...
use Botble\Ecommerce\Http\Requests\OrderRequest;
...
class OrderController extends BaseController
{
    ...
    public function store(OrderRequest $request, BaseHttpResponse $response)
    {
        ...
        if ($order) {
            ...
            $this->orderHistoryRepository->createOrUpdate([
                'action'      => 'create_order',
                'description' => __('New order :order_id', ['order_id' => get_order_code($order->id)]),
                'order_id'    => $order->id,
            ]);

            $this->orderHistoryRepository->createOrUpdate([
                'action'      => 'confirm_order',
                'description' => __('Order is verified by %user_name%'),
                'order_id'    => $order->id,
            ...
            if ($request->input('payment_status') === PaymentStatusEnum::COMPLETED) {
                $this->orderHistoryRepository->createOrUpdate([
                    ...
                    'description' => __('Payment was confirmed (amount :money) by %user_name%', [
                        'money' => format_price($order->amount - $order->discount_amount + $order->shipping_amount, $order->currency_id),
                    ]),
                    'order_id'    => $order->id,
                    'user_id'     => Auth::user()->getKey(),
                ]);
    ...
    public function edit($id)
    {
        ...
        page_title()->setTitle(__('Edit order :code', ['code' => get_order_code($id)]));
    ...
    public function postConfirm(Request $request, BaseHttpResponse $response)
    {
        ...
        return $response->setMessage(__('Confirm order successfully!'));
    }
    ...
    public function postResendOrderConfirmationEmail($id, BaseHttpResponse $response)
    {
        ...
        if (!$result) {
            return $response
                ->setError()
                ->setMessage(__('There is an error when sending email!'));
        }

        return $response->setMessage(__('Sent confirmation email successfully!'));
    }
    ...
    public function postCreateShipment(
        ...
        switch ($request->input('method')) {
            default:
                $result = $result->setMessage(__('Order is sent to shipping team'));
                break;
        }
        ...
        if (!$result->isError()) {
            ...
            $this->orderHistoryRepository->createOrUpdate([
                ...
                'description' => $result->getMessage() . ' ' . __('by %user_name%'),
                'order_id'    => $id,
                'user_id'     => Auth::user()->getKey(),
            ]);
            ...
            $shipmentHistoryRepository->createOrUpdate([
                ...
                'description' => __('Shipment is created from order %order_id%'),
    ...
    public function postCancelShipment($id, BaseHttpResponse $response)
    {
        ...
        $this->orderHistoryRepository->createOrUpdate([
            ...
            'description' => __('Shipment is cancelled by %user_name%'),
        ...
        return $response
            ...
            ->setMessage(__('Shipment is cancelled successfully!'));
    }
    ...
    public function postUpdateShippingAddress($id, AddressRequest $request, BaseHttpResponse $response)
    {
        ...
        return $response
            ...
            ->setMessage(__('Update shipping address successfully!'));
    }
    ...
    public function postCancelOrder($id, BaseHttpResponse $response)
    {
        ...
        $this->orderHistoryRepository->createOrUpdate([
            'action'      => 'cancel_order',
            'description' => __('Order is cancelled by %user_name%'),
            'order_id'    => $order->id,
            'user_id'     => Auth::user()->getKey(),
        ]);
    ...
    public function postConfirmPayment($id, BaseHttpResponse $response)
    {
        ...
        return $response->setMessage(__('Confirm payment successfully!'));
    }
    ...
    public function postRefund($id, RefundRequest $request, BaseHttpResponse $response)
    {
        ...
        if ($request->input('refund_amount') > ($order->payment->amount - $order->payment->refunded_amount)) {
            return $response
                ->setError()
                ->setMessage(__('Refund amount must be lower or equal :price', [
        ...
        foreach ($request->input('products', []) as $productId => $quantity) {
            ...
            if ($quantity > ($orderProduct->qty - $orderProduct->restock_quantity)) {
                ...
                $response
                    ->setError()
                    ->setMessage(__('Number of products refund is not valid!'));
                break;
            }
        }
        ...
        if (!$payment) {
            return $response
                ->setError()
                ->setMessage(__('Cannot found payment for this order!'));
        }
        ...
        if ($request->input('refund_amount', 0) > 0) {
            $this->orderHistoryRepository->createOrUpdate([
                'action'      => 'refund',
                'description' => __('Refund success :price', [
        ...
        return $response->setMessage(__('Refund successfully!'));
    }
    ...
    public function getReorder(Request $request, BaseHttpResponse $response)
    {
        ...
        page_title()->setTitle(__('Reorder'));
        ...
        foreach ($products as &$availableProduct) {
            $availableProduct->image_url = RvMedia::getImageUrl(Arr::first($availableProduct->images) ?? null, 'thumb', false, RvMedia::getDefaultImage());
    ...
    public function postSendOrderRecoverEmail($id, BaseHttpResponse $response)
    {
        ...
        try {
            ...
            return $response->setMessage(__('Sent email to remind about incomplete order successfully!'));
        } catch (Exception $exception) {
            return $response
                ->setError()
                ->setMessage(__('There is an error when sending email!'));
