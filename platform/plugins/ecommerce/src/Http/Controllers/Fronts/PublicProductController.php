...
class PublicProductController
{
    ...
    public function getProducts(Request $request, GetProductService $productService)
    {
        ...
        if ($query) {
            ...
            SeoHelper::setTitle(__('Search result "' . $query . '" '))->setDescription(__('Products: ') . '"' . $request->get('q') . '"');
        ...
        return Theme::scope('ecommerce.products-archive', compact('products'),
            'plugins/ecommerce::themes.products-archive')->render();
    }
    ...
    public function getProductVariation($id, Request $request, BaseHttpResponse $response)
    {
        ...
        if (!$product) {
            return $response->setError()->setMessage(__('0 products available'));
        }
    ...
    public function postCreateReview(ReviewRequest $request, BaseHttpResponse $response)
    {
        ...
        $request->merge(['user_id' => auth('customer')->user()->getAuthIdentifier()]);
