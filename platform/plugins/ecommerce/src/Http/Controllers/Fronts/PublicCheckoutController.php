...
class PublicCheckoutController
{
    ...
    public function getCheckoutInformation(
        ...
        return view('plugins/ecommerce::orders.checkout-information', compact(
            'token',
            'sessionCheckoutData',
    ...
    public function postSaveInformation(
        ...
        $sessionData = $request->input('address');
        ...
        try {
            ...
            if (!empty($address)) {
                ...
                $this->orderAddressRepository->create($addressData);
            }
        ...
        return $response->setNextUrl(route('public.checkout.payment', $token));
    }
    ...
    public function getCheckoutPayment(
        ...
        $orderTotal = Cart::rawTotal() - $promotionDiscountAmount;
        ...
        $defaultShippingMethod = $request->input('shipping_method',
            old('shipping_method', Arr::get($sessionData, 'shipping_method', Arr::first(array_keys($shipping)))));
        ...
        if (!empty($shipping)) {
            ...
            $defaultShippingOption = $request->input('shipping_option',
                old('shipping_option', Arr::get($sessionData, 'shipping_option', $defaultShippingOption)));
        }
        $shippingAmount = Arr::get($shipping, $defaultShippingMethod . '.' . $defaultShippingOption . '.price', 0);

        Arr::set($sessionData, 'shipping_method', $defaultShippingMethod);
        Arr::set($sessionData, 'shipping_option', $defaultShippingOption);
        Arr::set($sessionData, 'shipping_amount', $shippingAmount);
        OrderHelper::setOrderSessionData($token, $sessionData);
        ...
        return view('plugins/ecommerce::orders.checkout-payment', compact(
            'token',
            'shipping',
    ...
    public function postCheckout(
        ...
        if ($request->has('shipping_method') && !get_shipping_setting('free_ship',
            ...
            $shippingData = [
                ...
                'city'        => Arr::get($sessionData, 'city'),
                'weight'      => $weight,
                'order_total' => Cart::rawTotal() - $promotionDiscountAmount - $couponDiscountAmount,
            ];
        ...
        $request->merge([
            'amount'          => Cart::rawTotal() + $shippingAmount - $promotionDiscountAmount - $couponDiscountAmount,
    ...
    public function getPayPalStatus(
        ...
        if ($request->input('order_id')) {
            OrderHelper::processOrder($request->input('order_id'), $request->input('paymentId'));
        }
