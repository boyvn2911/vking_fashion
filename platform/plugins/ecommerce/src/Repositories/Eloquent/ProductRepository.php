...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function syncCategories($model, $categories = null)
    {
        ...
        try {
            ...
            $message = __('Sync categories completed.');
            $result = true;
        } catch (Exception $exception) {
            $message = __('Some error occurred when sync categories.');
            $result = false;
        }
    ...
    public function syncProductCollections($model, $productCollections = null)
    {
        ...
        try {
            ...
            $message = __('Sync product collections completed.');
            $result = true;
        } catch (Exception $exception) {
            $message = __('Some error occurred when sync product collections.');
            $result = false;
        }
    ...
    public function syncProducts($model, $products = null)
    {
        ...
        try {
            ...
            $message = __('Sync related products completed.');
            $result = true;
        } catch (Exception $exception) {
            $message = __('Some error occurred when sync related products.');
            $result = false;
        }
    ...
    public function getProducts(array $params)
    {
        ...
        $this->model = $this->model
            ->leftJoin('ec_product_categories', 'ec_products.category_id', '=', 'ec_product_categories.id')
                        ...
                        return $subQuery
                            ->where(function ($sub) {
                                ...
                                return $sub
                                    ...
                                    ->where('ec_products.start_date', '<=', now(config('app.timezone')))
                                    ->where('ec_products.end_date', '>=', Carbon::today(config('app.timezone')));
                            })
     ...
     * {@inheritDoc}
     */
    public function getProductByCollections(array $params)
    {
        $this->model = $this->originalModel;
        ...
        $this->model = $this->model
            ...
            ->where(function ($query) use ($params) {
                 ...
                 * @var Builder $query
                 */
                if (!$params['labels']['value_in']) {
                    return $query;
                }

                return $query
                    ->whereIn('ec_product_collections.' . $params['labels']['by'], $params['labels']['value_in']);
            });
        ...
        if ($filters['categories']) {
            $this->model = $this->model
                ...
                ->where(function ($query) use ($filters) {
                     ...
                     */
                    return $query
                        ->whereIn('ec_products.category_id', $filters['categories'])
                        ->orWhereIn('ec_product_category_product.category_id', $filters['categories']);
                });
        }
