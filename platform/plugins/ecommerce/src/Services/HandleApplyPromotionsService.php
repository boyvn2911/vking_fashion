            ...
            switch ($promotion->type_option) {
                case 'amount':
                    switch ($promotion->target) {
                        case 'amount-minimum-order':
                            if ($promotion->min_order_price <= Cart::rawTotal()) {
                        ...
                        case 'all-orders':
                            ...
                            break;
                        default:
                            if (Cart::count() >= $promotion->product_quantity) {
                                $promotionDiscountAmount += $promotion->value;
                            }
                            break;
                    }
                ...
                case 'percentage':
                    switch ($promotion->target) {
                        case 'amount-minimum-order':
                            if ($promotion->min_order_price <= Cart::rawTotal()) {
                                $promotionDiscountAmount += Cart::rawTotal() * $promotion->value / 100;
                            }
                            break;
                        case 'all-orders':
                            $promotionDiscountAmount += Cart::rawTotal() * $promotion->value / 100;
                    ...
                    break;
                case 'same-price':
                    if ($promotion->product_quantity > 1 && Cart::count() >= $promotion->product_quantity) {
                        foreach (Cart::content() as $cart_id => $item) {
