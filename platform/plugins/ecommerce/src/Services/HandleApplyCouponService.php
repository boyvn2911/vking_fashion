...
class HandleApplyCouponService implements ProduceServiceInterface
{
    ...
    public function execute(Request $request)
    {
        ...
        if ($discount->target === 'customer') {
            $discount_customers = $discount->customers()->pluck('customer_id')->all();
            if (!auth('customer')->check() ||
                !in_array(auth('customer')->user()->id, $discount_customers)
            ) {
                return [
                    'error'   => true,
                    'message' => trans('plugins/ecommerce::discount.invalid_coupon'),
            ...
            switch ($discount->type_option) {
                case 'amount':
                    switch ($discount->target) {
                        case 'amount-minimum-order':
                            if ($discount->min_order_price <= Cart::rawTotal()) {
                        ...
                        case 'all-orders':
                            ...
                            break;
                        default:
                            if (Cart::count() >= $discount->product_quantity) {
                                foreach ($cartItems as $cartId => $item) {
                                    $couponDiscountAmount += $discount->value;
                                }
                            }
                ...
                case 'percentage':
                    switch ($discount->target) {
                        case 'amount-minimum-order':
                            if ($discount->min_order_price <= Cart::rawTotal()) {
                                $couponDiscountAmount = Cart::rawTotal() * $discount->value / 100;
                            }
                            break;
                        case 'all-orders':
                            $couponDiscountAmount = Cart::rawTotal() * $discount->value / 100;
    ...
    public function getCouponData($couponCode, $sessionData)
    {
        ...
        $discount = $this->discountRepository
            ...
            ->where('type', 'coupon')
            ->where('start_date', '<=', now(config('app.timezone')))
                    ...
                    ->where(function ($sub) {
                        ...
                        return $sub
                            ...
                            ->where(function ($subSub) {
                                ...
                                return $subSub
                                    ->whereNull('end_date')
                                    ->orWhere('end_date', '>=', now(config('app.timezone')));
                            });
                    })
                    ->orWhere(function ($sub) use ($sessionData) {
                        ...
                        return $sub
                            ...
                            ->where(function ($subSub) use ($sessionData) {
                                ...
                                return $subSub
                                    ->whereNull('target')
                                    ->orWhere('target', Arr::get($sessionData, 'state'));
                            });
                    })
